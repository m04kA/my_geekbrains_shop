...
class OrederCreateView(CreateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.request.POST:
            ...
            busket_items = Basket.get_items(user=self.request.user)
            if len(busket_items):
                ...
                for num, form in enumerate(formset.forms):
                    ...
                    form.initial['price'] = busket_items[num].product.price
                busket_items.delete()
            else:
                formset = OrderFormSet()
...
def product_quantity_update_save(sender, update_filds, instance, **kwargs):
    if update_filds is 'quantity' or 'product':
        if instance.pk:
